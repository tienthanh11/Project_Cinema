...
import org.example.cinema_fullstack.models.dto.film.UpdateFilmDTO;
...
@Component
public class FilmUpdateValidator implements Validator {
    @Override
    public boolean supports(Class<?> clazz) {
        return UpdateFilmDTO.class.equals(clazz);
    }

    @Override
    public void validate(Object object, Errors errors) {
        UpdateFilmDTO film = (UpdateFilmDTO) object;
        ...
        // Image validation (optional if not uploading a new image)
        ...
        if (film.getDurations() <= 0) {
            errors.rejectValue("durations", "duration.null", "Thời lượng không được để trống và phải lớn hơn 0");
        }
        ...
        if (film.getDescriptions() == null || film.getDescriptions().trim().isEmpty()) {
            errors.rejectValue("descriptions", "descriptions.null", "Nội dung không được để trống");
        }
