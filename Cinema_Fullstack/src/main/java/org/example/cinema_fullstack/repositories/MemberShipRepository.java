...
public interface MemberShipRepository extends JpaRepository<Membership, Long> {
    ...
    @Query("SELECT m FROM Membership m " +
            ...
            "AND EXISTS (SELECT ar FROM AccountRole ar WHERE ar.account = m.account AND ar.role.name = 'ROLE_MEMBER') " +
            "AND NOT EXISTS (SELECT ar FROM AccountRole ar WHERE ar.account = m.account AND ar.role.name = 'ROLE_ADMIN')")
    Page<Membership> findAllWithPaginationFilteredByRole(Pageable pageable);
    ...
    @Query("SELECT m FROM Membership m " +
            ...
            "AND EXISTS (SELECT ar FROM AccountRole ar WHERE ar.account = m.account AND ar.role.name = 'ROLE_MEMBER') " +
            "AND NOT EXISTS (SELECT ar FROM AccountRole ar WHERE ar.account = m.account AND ar.role.name = 'ROLE_ADMIN')")
    Page<Membership> searchByNameFilteredByRole(@Param("name") String name, Pageable pageable);
